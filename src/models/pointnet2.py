...
class PointNet2SemSeg(nn.Module):
    def __init__(self, num_classes=8, input_channels=6):
    ...
    def forward(self, xyz):
        # xyz: [B, N, input_channels] where first 3 are coordinates
        B, N, C = xyz.shape
        
        # Split coordinates and features
        xyz_coords = xyz[:, :, :3].contiguous()
        xyz_features = xyz[:, :, 3:].contiguous() if C > 3 else None
        
        # Transpose for PointNet++ expected format [B, C, N]
        l0_xyz = xyz_coords.transpose(1, 2).contiguous()
        l0_points = xyz_features.transpose(1, 2).contiguous() if xyz_features is not None else None
        ...
        l0_points = self.fp1(l0_xyz, l1_xyz, torch.cat([l0_xyz, l0_points], 1) if l0_points is not None else l0_xyz, l1_points)
...
if __name__ == "__main__":
    ...
    # Dummy input: [B, N, C] where C = XYZ + RGB
    dummy_input = torch.randn(batch_size, num_points, input_channels)
    
    with torch.no_grad():
        output = model(dummy_input)
        print(f"Input shape: {dummy_input.shape}")
        print(f"Output shape: {output.shape}")
        print(f"Expected output shape: [batch_size, num_points, num_classes]")
